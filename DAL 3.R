...
str(gtd_df)
gtd_df <- gtd_df %>% 
  select(eventid, iyear, imonth, iday, summary, country, country_txt, region, region_txt, 
         provstate, city, crit1, crit2, crit3, doubtterr, success, suicide, attacktype1, 
         attacktype1_txt, attacktype2, attacktype2_txt, attacktype3, attacktype3_txt, 
         targtype1, targsubtype1_txt, targtype2, targtype2_txt, targtype3, targtype3_txt, 
         gname, gname2, gname3, nperps, nperpcap, motive, nkill, nkillus, nkillter, nwound,
         nwoundus, nwoundte, property, propextent, propextent_txt, propvalue, nhostkid, 
         nhostkidus, nhours, ndays, ransom, ransomamt, ransompaid, hostkidoutcome, 
...
AnswerA <- "The columns and rows show the countries where terrorist attacks have taken place, what year they took place,
the type of attack carried out, the assassination target, and attack success/failure."

# QuestionB: What years does the data set span? 
summary(gtd_df$iyear)
vector_numeric <- c(1970, 2018)
str (vector_numeric)

# Create Visualization of Number of Terrorist Attacks per Year ####
gtd_df %>% 
...
# Question 1: What trends do we see? Does anything look peculiar? ####
Answer1 <- "The number of attacks gradually increases until the year `93 when there is no data available."
...
Answer2 <- "1993, there is no data on terrorist attacks because the data collected was lost prior to START's compilation from other data collections." 
...
31060/191464=0.16222370785108*10

Answer4<- c(16.222370785108)
...
missing_casualties_df <- left_join(missing_casualties_data, gtd_casualties_df, (nkill), by = "iyear" )
...
Answer6<- incidents<- missing_casualties_data
...
gtd_casualties_df <- gtd_casualties_df %>% 
  mutate(iyear, casualties_per_inc = casualities / incidents, victims_killed_per_inc = victims_killed / incidents, victims_wounded_per_inc = victims_wounded / incidents) 

# Create a line plot of victims_killed_per_inc by year
gtd_casualties_df %>% 
  rename(year = iyear) %>% 
  ggplot(aes(x=year)) + 
  geom_line(aes(y=victims_killed/incidents), color = "red") 

# Create a line plot of victims_wounded_per_inc by year
gtd_casualties_df %>% 
  rename(year = iyear) %>% 
  ggplot(aes(x=year)) + 
  geom_line(aes(y=victims_wounded/incidents), color = "dark red") 

# Create a plot that Combines all these lines
casualties_plot <- gtd_casualties_df %>% 
  rename(year = iyear) %>% 
  ggplot(aes(x=year)) +
  geom_line(aes(y=victims_killed_per_inc), color = "yellow") + 
  geom_line(aes(y=Victims_wounded_per_inc), color = "green")
 
# Add lines to casualties_plot
casualties_plot <- casualties_plot +
  geom_linr(aes(y=victims_killed_per_inc, color = "yellow")) + # Add the correct ggplot geometric layer. 
  geom_line(aes(y=victims_wounded_per_inc, color = "green")) # Add the correct ggplot geometric layer. 
...
Answer7_plot <- "casualties_per_year  "
Answer7 <- "Describe the trends you see."
...
gtd_df %>% 
  ...
  ggplot(aes(x=ransomamt, y=ransompaid, color
  =region_txt))+
  geom_point()
